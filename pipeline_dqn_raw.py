...
if __name__ == '__main__':
    writer = SummaryWriter(log_dir=f'/mnt/nfs/chenyun/ServiceComposition/debug/node{max_node_num}-serset{max_ser_set}/{model}', filename_suffix='empty')
    ...
    if model == 'MHA':
        ...
        agent = DQNDecision(max_node_num, max_ser_set, device=device).to(device)
    optim = Adam(agent.parameters(), lr, weight_decay=0.01)
    for epoch in tqdm(range(epoch_size)):
        
        workflows, tasks, masks, constraints, topologicals = env.reset(bs, mode='tiny', norm=norm, static=True)
        ...
        ser_idx, ser_prob = agent.forward(tasks, constraints, masks, topologicals, writer, epoch)
        rewards = env.step(ser_idx.cpu().numpy())
        rewards = torch.from_numpy(rewards)
        if model == 'DQN':
            agent.reward_list.append(rewards)
            agent.post_process()
            if agent.buffer.occupied:
